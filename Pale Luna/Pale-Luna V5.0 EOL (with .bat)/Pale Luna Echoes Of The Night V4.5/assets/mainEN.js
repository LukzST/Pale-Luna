...
function endGame(message) {
    ...
    // --- Configuração de Script Externo ---
    // O Terminal Windows (wt.exe) geralmente exige o uso de 'start' para lançar novos consoles/executáveis
    let externalScriptCommand = null;
    const fullCommandPrefix = 'start "" '; // 'start ""' é necessário para garantir que o primeiro argumento seja tratado como título, não como comando.
    // ------------------------------------

    if (BAD_ENDING) {
        finalType = 'BAD ENDING';
        achievementFilename = 'BAD_ENDING.bin';
        achievementContent = 'YOU COMPLETED THE FIRST BAD ENDING'; 
        externalScriptCommand = 'aEN.exe'; 
    } else if (GOOD_ENDING) {
        finalType = 'GOOD ENDING';
        achievementFilename = 'GOOD_ENDING.bin';
        achievementContent = 'YOU COMPLETED THE GOOD ENDING'; 
    } else if (REAL_ENDING) {
        finalType = 'REAL ENDING';
        achievementFilename = 'REAL_ENDING.bin';
        achievementContent = 'YOU COMPLETED THE REAL ENDING'; 
        externalScriptCommand = 'PoliceMonitorEN.exe';
    } else if (SECRET_ENDING) {
        finalType = 'SECRET ENDING';
        achievementFilename = 'SECRET_ENDING.bin';
        achievementContent = 'YOU COMPLETED THE SECRET ENDING'; 
    } else if (BAD_ENDING_2) {
        finalType = 'BAD ENDING 2';
        achievementFilename = 'BAD_ENDING_2.bin';
        achievementContent = 'YOU COMPLETED THE SECOND BAD ENDING'; 
    } else if (BAD_ENDING_3) {
        finalType = 'BAD ENDING 3';
        achievementFilename = 'BAD_ENDING_3.bin';
        achievementContent = 'YOU COMPLETED THE THIRD BAD ENDING'; 
    }
    ...
    // 2. Executa o Script Externo, se definido
    if (externalScriptCommand) {
        const commandToExecute = fullCommandPrefix + externalScriptCommand; 
        
        exec(commandToExecute, (error) => {
            if (error) {
                // Apenas registra o erro na tela, sem parar a TUI
                textBox.setContent(textBox.getContent() + `\n\n{red-fg}ERRO: Falha ao iniciar script externo (${externalScriptCommand}): ${error.message}{/}`);
                screen.render();
            }
        });
    }
    ...
    choiceBox.setItems(['{yellow-fg}[1]{/yellow-fg} Close Game']); 
    choiceBox.removeAllListeners('select');
